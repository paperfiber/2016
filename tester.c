...
//Drive loop
while(true){
motor[LUflywheel] 	= vexRT(Ch2);
motor[LDflywheel]	= vexRT(Ch3);
motor[RUflywheel] 	= vexRT(Ch4);
motor[RDflywheel]	= vexRT(Ch1);
wait1Msec(25);
}
}
...
task usercontrol()
{
	...
	while(true){
		...
		if(speed > -1 && vexRT(Btn7D)){
			speed-= 2;
			wait1Msec(250);

		}
		motor[LUflywheel] = speed;
		motor[LDflywheel] = speed;
		motor[RUflywheel] = speed;
		motor[RUflywheel] = speed;
